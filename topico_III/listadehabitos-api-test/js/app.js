...
app.controller("apitestgetcontroller", ["$scope", "$http", function ($scope, $http) {
    ...
    $scope.listaDeHabitosApiGet = function () {
        if ($scope.id == "") {
            ...
            $http.get("http://localhost/programacao_web_I/unidade_III/topico_III/listadehabitos-rest-api/habito.php", {
                params: {
                    id: $scope.id
                }
            }).then(function (data) {
...
app.controller("apitestputcontroller", ["$scope", "$http", function ($scope, $http) {
    ...
    $scope.listaDeHabitosApiPut = (function () {
        $http.put("http://localhost/programacao_web_I/unidade_III/topico_III/listadehabitos-rest-api/habito.php", {
            id: $scope.id,
            nome: $scope.nome,
            status: $scope.status
        }).then(function (data) {
