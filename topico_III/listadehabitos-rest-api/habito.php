...
{
    // cria uma cláusula WHERE com os parâmetros que foram recebidos através da requisição HTTP get
    $queryWhere = " WHERE ";
    ...
    if ($resultado->num_rows > 0) {
        ...
        while ($registro = $resultado->fetch_assoc()) {
            // Monta um objeto Json através de um array associativo ou seja, indexado através de strings
            $jsonHabito = array();
        ...
        // Transforma o array com os resultados da queryem um array Json e imprime-o na página
...
function f_update_habito()
{
    ...
    $sql = " UPDATE habito "
        . " SET status = '" . $status . "' " . " , nome = '" . $nome . "'" . " WHERE id = " . $id;
...
function f_delete_habito()
{
    ...
    // retorna o Registro atualizado
    echo json_encode("Registro excluído!");
    $conn->close();
}

// A variável de servidor REQUEST_METHOD contém o nome do método HTTP através
// qual o arquivo solicitado foi acessado
$metodo = $_SERVER['REQUEST_METHOD'];

// Verifica qual ação a ser tomada de acordo com o método HTTP que foi utilizado para acessar
// este recurso
switch ($metodo) {
        // Se foi GET deve consultar
    case "GET":
        f_select_habito();
        break;
        // Se foi POST deve inserir
    case "POST":
        f_insert_habito();
        break;
        // Se foi put deve alterar
    case "PUT":
        f_update_habito();
        break;
        // Se foi delete deve excluir
    case "DELETE":
        f_delete_habito();
        break;
}
